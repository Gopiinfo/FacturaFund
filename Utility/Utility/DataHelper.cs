...
namespace Utility
{
    public class DataHelper
    {
        internal static T GetRecord<T>(string spName, List<ParamInfo> parameters, string strFunctionName)
        {
            ...
            try
            {
                using (SqlConnection oCon = new SqlConnection(Common.ConnectionString()))
                {
                    oCon.Open();
        ...
        public static async Task<List<T>> GetRecords<T>(string spName, List<ParamInfo> parameters, string strFunctionName)
        {
        ...
        public static long ExecuteQuery(string spName, List<ParamInfo> parameters, string strFunctionName)
        {
            long iStatus = 0;
            try
            {
        ...
        public static async Task<long> ExecuteMultiDataTableWithLongOutput(string procName, Dictionary<string, DataTable> tables, string strFunctionName)
        {
            long oRec = 0;
            try
            {
                using (SqlConnection oCon = new SqlConnection(Common.ConnectionString()))
                {
                    ...
                    using (SqlCommand command = new SqlCommand())
                    {
                        ...
                        {
                            SqlParameter tvpParam = command.Parameters.AddWithValue("@" + table.Key, table.Value);
                        ...
                        using (var reader = command.ExecuteReader())
                        {
                            ...
                            while (reader.Read())
                            {
                                int customerId = reader.GetInt32(customerIdOrdinal); // Use appropriate type
                                oRec = customerId;
                            }
                        }
            ...
            {
                LogErrorinDB("", "DataHelper", "ExecuteMultiDataTableWithLongOutput" + strFunctionName.ToString(), ex.Message.ToString() + "\t\t" + procName);
            }
            if (oRec == 0)
                return -1;
            else
